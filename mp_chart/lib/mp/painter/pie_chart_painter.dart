...
class PieChartPainter extends PieRadarChartPainter<PieData?> {
  ...
  List<double?> _drawAngles = List.filled(1, null, growable: false);

  /// array that holds the absolute angle in degrees of each slice
  List<double?> _absoluteAngles = List.filled(1, null, growable: false);
  ...
  void calcAngles() {
    ...
    if (_drawAngles.length != entryCount) {
      _drawAngles = List.filled(entryCount, null, growable: false);
    } else {
    ...
    if (_absoluteAngles.length != entryCount) {
      _absoluteAngles = List.filled(entryCount, null, growable: false);
    } else {
      for (int i = 0; i < entryCount; i++) {
        _absoluteAngles[i] = 0;
      }
    ...
    List<double?> minAngles = List.filled(entryCount, null, growable: false);
  ...
  double getRequiredLegendOffset() {
    ...
    var offset =
        legendRenderer!.legendLabelPaint!.text?.style?.fontSize! ?? 1 * 2.0;
